...
export class ThreatAnalysisService {
  ...
  async analyzeUrl(url: string): Promise<ThreatAnalysisResult> {
    ...
    try {
      ...
      // 1. VirusTotal Check
      const vtResult = await threatIntelligenceService.checkVirusTotal(url);
      ...
      const gsbResult = await threatIntelligenceService.checkGoogleSafeBrowsing(url);
      ...
      // 3. PhishTank Check
      const ptResult = await threatIntelligenceService.checkPhishTank(url);
      ...
      const technicalDetails: TechnicalDetails = {
        domainAge: whoisInfo.domainAgeDays != null ? String(whoisInfo.domainAgeDays) : domainInfo.age || 'N/A',
        ...
        redirects: redirectInfo.hops != null ? String(redirectInfo.hops) : '0',
        responseTime: 'N/A',
        whois: whoisInfo,
        redirect: redirectInfo,
      };
